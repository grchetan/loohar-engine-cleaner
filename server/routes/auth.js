...
router.post('/firebase', async (req, res) => {
  try {
    ...
    if (!decoded) return res.status(401).json({ success: false, message: 'Invalid Firebase token' });
...
router.put('/me/address', protect, async (req, res) => {
  try {
    ...
    const { _id, label, fullName, phone, line1, line2, city, state, pincode, isDefault } = req.body;

    if (isDefault) user.addresses.forEach(a => a.isDefault = false);

    if (_id) {
      const addr = user.addresses.id(_id);
      if (addr) Object.assign(addr, { label, fullName, phone, line1, line2, city, state, pincode, isDefault });
    } else {
